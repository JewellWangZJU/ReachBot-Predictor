...
from .reachbot_score import dice_score, iou_score
...
def log_metrics(
    metrics: Union[dict, list], log_to_wandb: bool = True, step: Optional[int] = None
) -> float:
    """
    Prints the metrics.
    """
        ...
        if has_full_mask:
            for key in metrics[0]["full_mask"].keys():
                if "intersection" not in key and "union" not in key:
                    ...
                    wandb_metrics["Avg individual masks/" + key] = metrics_avg_indiv[
                        key
                    ]
                    wandb_metrics["Std individual masks/" + key] = metrics_std_indiv[
                        key
                    ]
        else:
